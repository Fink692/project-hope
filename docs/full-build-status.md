...
|---|---|---|
| Identity, organizations, RBAC, tenant isolation | Case-normalized email users, Django password hashing, sessions, expiring native tokens, non-enumerating password recovery, built-in TOTP enrollment/challenge, encrypted MFA secrets, one-time recovery codes, single-use login challenges, security-version revocation, organizations, five membership roles, scoped lookups, append-only access events | Optional OIDC/SSO federation, remote device/session inventory, organization-approved privileged-session policy, phishing-resistant factor option, independent authorization/session testing |
...
| CRM | Organization-scoped contacts, households, relationships, interactions, consent, sensitivity fields, guided CSV/XLSX template and preview, reviewed create/fill-missing import, spreadsheet-safe export, contact correction, deterministic duplicate review, and history-preserving merge UI/API | Real charity sample-spreadsheet acceptance, charity vocabulary validation, and production-scale migration rehearsal |
...
| Accessibility | Semantic responsive web foundation, visible focus, reduced-motion support, transformation/review records, and [v1.8 automated CRM migration evidence](accessibility-audit-v1.8.md) | Manual WCAG 2.2 AA audit, screen-reader/keyboard/zoom testing, tagged export validation, testing with users with disabilities |
...
| Operations | Development/production Compose, Gunicorn/Caddy, worker, local models, backup/restore scripts, setup helpers, fail-closed production MFA/cache configuration, operator MFA reset, and encryption-key rotation workflow | Live production environment, organization-owned secrets, pinned container digests, monitored backups, completed restore and recovery drills, incident/support rota |
