...
Before production use, operators must set separate strong Django and MFA-encryption secrets, enable HTTPS, configure trusted hosts and CSRF origins, keep the shared Valkey security cache available, require built-in MFA enrollment, rehearse MFA recovery and key rotation, restrict database access, encrypt disks/backups, test restore procedures, and review the threat model.
